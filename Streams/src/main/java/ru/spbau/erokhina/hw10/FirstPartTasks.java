...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getName))
                .values().stream().filter(lst -> lst.size() > 1).count();
    }
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(Comparator
                .comparing(album -> album.getTracks().stream().
                        max(Comparator.comparing(Track::getRating)).orElse(new Track("", 0)).getRating()));
    }

    // Список альбомов, отсортированный по убыванию среднего рейтинга его треков (0, если треков нет)
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparing((Album album) -> album.getTracks().stream()
                .mapToInt(Track::getRating).average().getAsDouble()).reversed())
                .collect(Collectors.toList());
    }
